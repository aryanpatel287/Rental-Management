  ...
  return (
    <>
      ...
      <header className="sidebar-mobile-bar">
        <Link to="/dashboard" className="sidebar-mobile-bar__brand" aria-label="Home">
      ...
      <aside className={`sidebar ${isOpen ? 'sidebar--open' : ''}`}>
        {/* Brand Brand logo */}
        <Link to="/dashboard" className="sidebar__header" onClick={closeSidebar} aria-label="Home">
          <i className="ri-command-fill" aria-hidden="true" />
          <span>Goat Template</span>
        </Link>
        ...
        <nav className="sidebar__nav" role="navigation" aria-label="Main Navigation">
          ...
          {entities.length > 0 && (
            <>
              <div style={{ height: '1px', backgroundColor: 'var(--color-hairline, #f0f0f3)', margin: '12px 8px' }} />
              {entities.map((ent) => (
                <Link
                  key={ent.slug}
                  to={`/crud/${ent.slug}`}
                  ...
                  onClick={closeSidebar}
                  aria-current={isActive(`/crud/${ent.slug}`) ? 'page' : undefined}
                >
        ...
        {user && (
          <div className="sidebar__footer">
            <div className="sidebar__user-plate" title={`Logged in as ${user.name}`}>
